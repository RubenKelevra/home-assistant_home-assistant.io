...
In this release, we welcome the `select` entity to the Home Assistant family. The select entity is a close relative of the dropdown helper (also known as
`input_select`).
...
this. Consider a big automation, with a whole bunch of triggers. But how would you know which of those triggers actually triggered the automation?

You can now assign an `id` to your triggers that are passed into automation when
...
we added the ability to debug automations. In this release, we've made these same powerful tools available for scripts!
...
is converting the state of a sensor or text to a datetime. In this
...
Additionally, access to Home Assistant from the same IP address as a trusted proxy will be
...
The statistics table is Home Assistant's data table that is not exposed or used by Home Assistant yet and is part of an alpha feature that is in
development. However, it does exist and you might already want to check it out or find a use for it.
...
Kuler Skylights no longer supports the deprecated `white_value` attribute for its lights. Use the `rgbw_color` attribute instead.
...
This was unintended and an undocumented functionality that leads to unexpected behavior.
...
The AirQuality platform has been marked as deprecated. The `air_quality` entities will be deleted and replaced with `sensor` entities.
You need to update your automations and dashboards if you have been using these `air_quality` entities in those.
...
previously had configured multiple PVPC sensors monitoring prices for more than one of the old tariffs, only the first one will survive. This means if you have any automation or script that depends on these removed sensors, you might need to adjust them.
...
via the user interface only. Existing platform YAML config will automatically be imported into the user interface on upgrade and can be safely removed from the YAML configuration after the upgrade has been completed.
